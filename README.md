...
___This is flawed.___ Our current pins abide by this rule,
but rely on commands being in `PATH` for the update script to work.
Update script must be derivations so they can declare their dependencies
and they get built before running the update script.
...
### Converting submodule sources to pins

A work in progress. Some insights we gained already:

* Use `builtins.fetchGit`, not `fetchgit` from nixpkgs.
  The latter cannot handle private git repositories,
  because it is a derivation, which is built in a sandbox
  that cannot access our ssh agent.
  `builtins.fetchGit` can.
  <https://discourse.nixos.org/t/fetchgit-vs-fetchgit/23325/3>
* `builtins.fetchGit` seemed to always pop up an SSH agent confirmation popup,
  i.e. always tries to hit the git server.
  However this seems to be more subtle than that.
  It does not try to connect if you supply a `ref` (which is otherwise
  irrelevant).
  Then it can even access older git commits without network access!
  This makes me sort of question whether I'm really pinning the commit by using `rev`,
  but hopefully this is 'just' some git cache inside Nix that I know nothing
  about?
* Pass `--read-write-mode` to `nix-instantiate`,
  otherwise you get very unclear error messages about `*.drv` being an invalid
  path when it is not already in the nix store.
  In other words: Import From Derivation is enabled by default,
  but still won't work if you don't pass `--read-write-mode`.
  This was a painful lesson.
  The flag is used without any explanation in
  <https://nix.dev/manual/nix/2.28/language/import-from-derivation>,
...
based on <https://phip1611.de/blog/accessing-network-from-a-nix-derivation/>:
    ...
    stdenv.mkDerivation {
        ...
        src = builtins.fetchGit {
            ...
            # Without it, an SSH connection is made to thee even if this package is
        ...
        outputHash = ""; # "sha256-4nWrv9SSNiBTIO4pRU1TYCRRkupjEcZIcmcE8gQ7YxE=";
    }
...
non-nix code to build. Although a capsule-from-a-pin can also 'build' something
